...
void loop()
{
    ...
    Wire.requestFrom(MPU, 6, true);  // Read 6 registers total, each axis value
                                     // is stored in 2 registers
    ...
    Wire.endTransmission(false);
    Wire.requestFrom(MPU, 6, true);  // Read 4 registers total, each axis value
                                     // is stored in 2 registers
    GyroX = (Wire.read() << 8 | Wire.read()) /
            131.0;  // For a 250deg/s range we have to divide first the raw
...
void calculateImuError()
{
    ...
    while (c < 200)
    {
        ...
        AccErrorY =
            AccErrorY +
            ((atan(-1 * (AccX) / sqrt(pow((AccY), 2) + pow((AccZ), 2))) * 180 /
              PI));
        c++;
    }
...
void setupImu()
{
    ...
    Wire.beginTransmission(
        MPU);  // Start communication with MPU6050 // MPU=0x68
