...
void MultiStepper::moveTo(long absolute[])
{
    ...
    if (longestTime > 0.0)
    {
        ...
        for (i = 0; i < _num_steppers; i++)
        {
            ...
            _steppers[i]->moveTo(
                absolute[i]);  // New target position (resets speed)
...
boolean MultiStepper::run()
{
    ...
    for (i = 0; i < _num_steppers; i++)
    {
        ...
        // Caution: it has een reported that if any motor is used with
        // acceleration outside of MultiStepper, this code is necessary, you get
        // strange results where it moves in the wrong direction for a while
        // then slams back the correct way.
#if 0
	else
	{
