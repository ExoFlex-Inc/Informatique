...
#define SERVO_INDEX_TO_TIMER(_servo_nbr)                                       \
    ((timer16_Sequence_t) (_servo_nbr /                                        \
                           SERVOS_PER_TIMER))  // returns the timer controlling
                                               // this servo
...
#define SERVO(_timer, _channel)                                                \
    (servos[SERVO_INDEX(_timer, _channel)])  // macro to access servo class by
                                             // timer and channel
                   ...
                   uint8_t intFlag)
{
    ...
    else
    {
        ...
        {  // allow a few ticks to ensure the next
           // OCR1A not missed
            tc->COUNT16.CC[channel].reg =
        ...
        currentServoIndex[timer] =
            -1;  // this will get incremented at the end of the refresh period
                     ...
                     uint8_t gcmForTimer, uint8_t intEnableBit)
{
    ...
    // Set the prescaler factor to GCLK_TC/16.  At nominal 48 MHz GCLK_TC this
    // is 3000 ticks per millisecond
...
Servo::Servo()
{
    ...
    {
        this->servoIndex =
            ServoCount++;  // assign a servo index to this instance
...
uint8_t Servo::attach(int pin, int min, int max)
{
    ...
    if (this->servoIndex < MAX_SERVOS)
    {
        ...
        this->min =
            (MIN_PULSE_WIDTH - min) / 4;  // resolution of min/max is 4 us
