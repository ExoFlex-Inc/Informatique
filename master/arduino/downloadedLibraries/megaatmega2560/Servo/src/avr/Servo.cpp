...
#define usToTicks(_us)                                                         \
  ((clockCyclesPerMicrosecond() * _us) /                                       \
   8) // converts microseconds to tick (assumes prescale of 8)  // 12 Aug 2009
...
Servo::Servo() {
  if (ServoCount < MAX_SERVOS) {
    ...
    servos[this->servoIndex].ticks =
        usToTicks(DEFAULT_PULSE_WIDTH); // store default values  - 12 Aug 2009
  } else
...
void Servo::write(int value) {
  if (value < MIN_PULSE_WIDTH) { // treat values less than 544 as angles in
