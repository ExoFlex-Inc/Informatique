...
public:
  ServoImpl(PinName _pin) { pin = new mbed::DigitalOut(_pin); }
...
#define SERVO_MIN()                                                            \
  (MIN_PULSE_WIDTH - this->min) // minimum value in us for this servo
#define SERVO_MAX()                                                            \
  (MAX_PULSE_WIDTH - this->max) // maximum value in us for this servo

#define TRIM_DURATION                                                          \
  15 // callback overhead (35 us) -> 15 us if toggle() is called after starting
     // the timeout
...
bool Servo::attached() { return servos[this->servoIndex] != NULL; }
