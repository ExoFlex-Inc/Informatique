...
uint32_t group_pins[3][NRF_PWM_CHANNEL_COUNT] = {
    ...
    {NRF_PWM_PIN_NOT_CONNECTED, NRF_PWM_PIN_NOT_CONNECTED,
     NRF_PWM_PIN_NOT_CONNECTED, NRF_PWM_PIN_NOT_CONNECTED}};
static uint16_t seq_values[3][NRF_PWM_CHANNEL_COUNT] = {
...
uint8_t Servo::attach(int pin) { return this->attach(pin, 0, 2500); }
...
void Servo::detach() { servos[this->servoIndex].Pin.isActive = false; }
...
void Servo::writeMicroseconds(int value) {
  ...
  NRF_PWM_Type *PWMInstance =
      instance == 0 ? NRF_PWM0 : (instance == 1 ? NRF_PWM1 : NRF_PWM2);
  // configure PWM instance and enable it
  seq_values[instance][channel] = value | 0x8000;
  nrf_pwm_sequence_t const seq = {seq_values[instance],
                                  NRF_PWM_VALUES_LENGTH(seq_values), 0, 0};
  nrf_pwm_pins_set(PWMInstance, group_pins[instance]);
  nrf_pwm_enable(PWMInstance);
  nrf_pwm_configure(PWMInstance, NRF_PWM_CLK_125kHz, NRF_PWM_MODE_UP,
                    2500); // 20ms - 50Hz
