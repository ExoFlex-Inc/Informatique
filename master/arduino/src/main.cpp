...
#define tightening_step_resolution                                             \
    200  // Set desired move: 200 steps (in quater-step resolution that's one
         // rotation)
...
void sendMsg()
{
    ...
    doc["TighteningAngle"] =
        map(tighteningCurrentAngle, 0, tightening_step_resolution, 0, 180);
...
void readMsg()
{
    ...
    if (!parse_msg.isNull())
    {
        ...
        tighteningCurrentAngle =
            map(tighteningCurrentAngle, 0, 180, 0, tightening_step_resolution);
    }
    ...
    if (!parse_msg.isNull())
    {
        dorsiflexMotorCurrentAngle =
            map(doc["DorsiflexAngle"], 0, 180, 1000, 2000);
    }
    ...
    if (!parse_msg.isNull())
    {
        eversionMotorCurrentAngle =
            map(doc["EversionAngle"], 0, 180, 1000, 2000);
    }
}

void motorMove(const char* motor, int power)
{
    // Remap power percent to motor speed values(0 = full reverse, 90 = stop,
    // 180 = full forward)
    if (power >= 100)
    {
        power = 180;
    ...
    if (motor == "dorsiflex")
    {
        ...
        // Update Encoder
        dorsiflexState = digitalRead(
            dorsiflex_channel_A);  // Reads the "current" state of the channel_A
        ...
        dorsiflexLastState =
            dorsiflexState;  // Updates the previous state of the
                             // channel_A with the current state
    }
    else if (motor == "eversion")
    {
        ...
        // Update Encoder
        eversionState = digitalRead(
            eversion_channel_A);  // Reads the "current" state of the channel_A
        ...
        eversionLastState = eversionState;  // Updates the previous state of the
                                            // channel_A with the current state
    }
...
void setup()
{
    ...
    stepper_tight.setAcceleration(
        500);  // Set acceleration value for the stepper
