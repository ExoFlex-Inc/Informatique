...
if (workbox) {
    ...
    const buildStrategy = () => {
        if (data.networkTimeoutSeconds > 0) {
            return new workbox.strategies.NetworkFirst({ cacheName, networkTimeoutSeconds: data.networkTimeoutSeconds });
        } else {
    ...
    const manifestURLs = manifest.map(entry => new URL(entry.url, self.location.href).href);

    // Cache resources during service worker installation
    self.addEventListener('install', (event) => {
        event.waitUntil(
            caches.open(cacheName).then((cache) => cache.addAll(manifestURLs.map(url => new Request(url, { credentials: data.credentials }))))
        );
    });
