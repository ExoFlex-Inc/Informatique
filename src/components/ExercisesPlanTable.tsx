...
const ExercisesPlanTable: React.FC<ExercisesPlanTableProps> = ({
    ...
    const exerciseOptions = ["Right Extension Knee", "Right Dorsiflexion Ankle", "Right Eversion Ankle",
        "Left Extension Knee", "Left Dorsiflexion Ankle", "Left Eversion Ankle"
    ];
    ...
    const handleInputChange = (setIndex: number, event, exerciseIndex?: number) => {
        ...
        if (['repetitions', 'rest', 'target_angle', 'target_torque', 'time'].includes(name) && parsedValue < 0) {
            parsedValue = 0;
        }
            ...
            <tbody className="bg-white">
              { set.movement.map((element, exerciseIndex) => (
                <tr key={exerciseIndex}>
                  {exerciseIndex === 0 && (
                      <td rowSpan={0} className="px-6 py-4 whitespace-nowrap text-right">
                          <input ref={(el) => (checkboxRefs.current[setIndex] = el)}
                          type="checkbox"
                          className="mr-4"
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    <select
                      ...
                      onChange={(e) => handleInputChange(setIndex, e, exerciseIndex)}
