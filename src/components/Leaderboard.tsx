...
export default function Leaderboard({ users, currentUser }: LeaderboardProps) {
  const [sortBy, setSortBy] = useState<'current_streak' | 'longest_streak'>('current_streak');
  const [isCurrentUserPinned, setIsCurrentUserPinned] = useState(true);
  const listRef = useRef<HTMLUListElement>(null);

  // Sort users based on selected streak type
  const sortedUsers = [...users].sort((a, b) => b.stats[sortBy] - a.stats[sortBy]);

  // Find current user rank within the sorted list
  const currentUserRank = sortedUsers.findIndex(user => user.user_id === currentUser.user_id);
  ...
  return (
    <Grid item xs={12} md={8}>
      <Paper elevation={3} sx={{ 
        ...
        <Typography variant="h4" gutterBottom align="center" sx={{ color: '#2196f3', fontWeight: 'bold', mb: 3 }}>
          LEADERBOARD
        </Typography>
        <Box sx={{ 
          display: 'flex', 
          ...
          <ButtonGroup variant="contained" sx={{ '& > *': { mx: 1 } }}>
            <Button
              ...
              sx={{
                backgroundColor: sortBy === 'current_streak' ? '#2196f3' : '#e0e0e0',
                color: sortBy === 'current_streak' ? 'white' : 'black',
                '&:hover': {
                  backgroundColor: sortBy === 'current_streak' ? '#1976d2' : '#d5d5d5',
                },
              }}
            >
              CURRENT STREAK
            ...
            <Button
              ...
              sx={{
                backgroundColor: sortBy === 'longest_streak' ? '#2196f3' : '#e0e0e0',
                color: sortBy === 'longest_streak' ? 'white' : 'black',
                '&:hover': {
                  backgroundColor: sortBy === 'longest_streak' ? '#1976d2' : '#d5d5d5',
                },
              }}
            >
              LONGEST STREAK
        ...
        <List ref={listRef} sx={{ width: '100%', bgcolor: 'transparent', overflowY: 'auto', flexGrow: 1 }}>
          {sortedUsers.map((user, index) => (
            <ListItem key={user.id} sx={{ 
              ...
              <ListItemText
                primary={
                  <Typography component="span" variant="body1" sx={{ fontWeight: 'bold', color: 'black' }}>
