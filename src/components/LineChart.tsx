...
registerLicense('Ngo9BigBOggjHTQxAR8/V1NHaF5cWWdCf1FpRmJGdld5fUVHYVZUTXxaS00DNHVRdkdgWH5fdnVTRWhYVE11XkU=');
...
const LineChart = () => {
    ...
    if (profile) {
      ...
      if (error) {
        ...
        data.forEach((item: { angle_data: any; created_at: string })=> {
          if (item.angle_data && item.created_at && item.angle_data[selectedData]) {
            item.angle_data[selectedData].forEach((entry: { data: number; time: string }) => {
              newData.push({
                angle: entry.data,
                time: entry.time,
  ...
  useEffect(() => {
    if (chartData.length > 0) {
      ...
      if (timelineFormat === 'y-MMM') {
        ...
        newData = chartData.filter(entry => {
  ...
  const lineCustomSeries = [
    {
      dataSource: chartData.map((entry: { angle: number; time: string; created_at: string }) => ({
  ...
  return (
    <>
      ...
      >
        ...
        <SeriesCollectionDirective>
          <SeriesDirective dataSource={lineCustomSeries[0].dataSource} xName="x" yName="y" type="Line" marker={{ visible: true }} />
      </SeriesCollectionDirective>
      </ChartComponent>
      
      <div className='text-black'>
        Start Date: 
        <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className='mr-8' />
        End Date: 
