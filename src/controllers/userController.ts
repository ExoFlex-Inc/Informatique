...
export const getUserProfile = async (req: Request, res: Response) => {
  ...
  if (error) {
    return res.status(500).json({ error: `Error fetching user profile: ${error.message}` });
  }
...
export const updateUserProfile = async (req: Request, res: Response) => {
  ...
  if (authError) {
    return res.status(500).json({ error: `Error updating user authentication data: ${authError.message}` });
  }
  ...
  if (profileError) {
    return res.status(500).json({ error: `Error updating user profile: ${profileError.message}` });
  }
...
async function deleteOldImage(path: string | null) {
  if (path) {
    const { error: deleteError } = await supaClient.storage.from("avatars").remove([path]);
    if (deleteError) {
...
export const downloadAvatar = async (req: Request, res: Response) => {
  ...
  if (error) {
    return res.status(500).json({ error: `Error downloading avatar: ${error.message}` });
  }
...
export const uploadAvatar = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (profileError) {
    return res.status(500).json({ error: `Error fetching user profile: ${profileError.message}` });
  }
  ...
  if (uploadError) {
    return res.status(500).json({ error: `Error uploading avatar: ${uploadError.message}` });
  }
  ...
  if (updateError) {
    return res.status(500).json({ error: `Error updating user profile: ${updateError.message}` });
  }
