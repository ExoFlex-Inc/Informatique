...
void CanMotorServo_BufferAppendInt16(uint8_t* buffer, int16_t number,
                                     int16_t* index);
void CanMotorServo_BufferAppendInt32(uint8_t* buffer, int32_t number,
                                     int32_t* index);
...
void CanMotorServo_SetOrigin(uint8_t controller_id)
{
    ...
    CanMotorServo_Transmit(
        controller_id | ((uint32_t) CAN_PACKET_SET_ORIGIN_HERE << 8),
        &set_origin_mode, FDCAN_DLC_BYTES_1);
}
...
void CanMotorServo_SetPos(uint8_t controller_id, float pos)
{
    ...
    uint8_t buffer[4];
    CanMotorServo_BufferAppendInt32(buffer, (int32_t) (pos * 10000.0),
                                    &send_index);
...
void CanMotorServo_SetSpeed(uint8_t controller_id, float rpm)
{
    ...
    CanMotorServo_Transmit(controller_id | ((uint32_t) CAN_PACKET_SET_RPM << 8),
                           buffer, FDCAN_DLC_BYTES_4);
}
                               ...
                               int16_t RPA)
{
    ...
    CanMotorServo_Transmit(
        controller_id | ((uint32_t) CAN_PACKET_SET_POS_SPD << 8), buffer,
        FDCAN_DLC_BYTES_8);
}
...
void CanMotorServo_SetCurrentBreak(uint8_t controller_id, float current)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    CanMotorServo_BufferAppendInt32(buffer, (int32_t) (current * 1000.0),
                                    &send_index);
    CanMotorServo_Transmit(
...
void CanMotorServo_SetDuty(uint8_t controller_id, float duty)
{
    int32_t send_index = 0;
    uint8_t buffer[4];
    CanMotorServo_BufferAppendInt32(buffer, (int32_t) (duty * 100000.0),
                                    &send_index);
    CanMotorServo_Transmit(
        controller_id | ((uint32_t) CAN_PACKET_SET_DUTY << 8), buffer,
        FDCAN_DLC_BYTES_4);
}
