...
void PeriphSolenoid_DeactivateSolenoid(TIM_HandleTypeDef* htim,
                                       uint32_t channel, GPIO_TypeDef* GPIO1,
                                       uint16_t pin1, GPIO_TypeDef* GPIO2,
                                       uint16_t pin2);
uint32_t PeriphSolenoid_SetDutyCycle(TIM_HandleTypeDef* htim, uint32_t channel,
                                     float duty_cycle);
void     PeriphSolenoid_StartPWM(TIM_HandleTypeDef* htim, uint32_t channel,
                                 uint32_t compare_value);
...
bool PeriphSolenoid_UnlockChangeSide()
{
    ...
    if (!isChangeSideFree)
    {
        uint32_t compare_value = PeriphSolenoid_SetDutyCycle(
            PS_CS_HTIM, PS_CS_CHANNEL, SOLENOID_PWM_VALUE);

        // Activate Soleinoid
        PeriphSolenoid_ActivateSolenoid(PS_CS_HTIM, PS_CS_CHANNEL, PS_CS_GPIO4,
                                        PS_CS_PIN4, PS_CS_GPIO3, PS_CS_PIN3,
                                        compare_value);
        ...
        isChangeSideFree)
    {
        ...
        PeriphSolenoid_DeactivateSolenoid(PS_CS_HTIM, PS_CS_CHANNEL,
                                          PS_CS_GPIO4, PS_CS_PIN4, PS_CS_GPIO3,
                                          PS_CS_PIN3);
    }
...
bool PeriphSolenoid_UnlockEversion()
{
    ...
    {
        uint32_t compare_value = PeriphSolenoid_SetDutyCycle(
            PS_EV_HTIM, PS_EV_CHANNEL, SOLENOID_PWM_VALUE);

        // Activate Soleinoid
        PeriphSolenoid_ActivateSolenoid(PS_EV_HTIM, PS_EV_CHANNEL, PS_EV_GPIO1,
                                        PS_EV_PIN1, PS_EV_GPIO2, PS_EV_PIN2,
                                        compare_value);
    ...
    if (currentTime - lastActivationTime >= LOCK_WAITING_DELAY &&
        isEversionFree)
    {
        PeriphSolenoid_DeactivateSolenoid(PS_EV_HTIM, PS_EV_CHANNEL,
                                          PS_EV_GPIO1, PS_EV_PIN1, PS_EV_GPIO2,
                                          PS_EV_PIN2);
    }
...
void periphSolenoid_FootThight()
{
    ...
    {
        uint32_t compare_value = PeriphSolenoid_SetDutyCycle(
            PS_MF_HTIM, PS_MF_CHANNEL, MOTOR_PWM_VALUE);
                                     ...
                                     float duty_cycle)
{
    ...
    uint32_t compare_value = (uint32_t) ((duty_cycle / 100.0f) * (arr + 1));
