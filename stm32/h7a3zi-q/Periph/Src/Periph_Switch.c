...
{
	if (HAL_GPIO_ReadPin(PS_GPIO_EXTENSION_DOWN, PS_PIN_EXTENSION_DOWN) == GPIO_PIN_RESET)
	{
	  return true;
	}
	return false;
...
{
	if (HAL_GPIO_ReadPin(PS_GPIO_DORSIFLEXION_UP, PS_PIN_DORSIFLEXION_UP) == GPIO_PIN_RESET)
	{
	  return true;
	}
	return false;
...
{
	if (HAL_GPIO_ReadPin(PS_GPIO_DORSIFLEXION_DOWN, PS_PIN_DORSIFLEXION_DOWN) == GPIO_PIN_RESET)
	{
	  return true;
	}
	return false;
...
{
	if (HAL_GPIO_ReadPin(PS_GPIO_EVERSION_LEFT, PS_PIN_EVERSION_LEFT) == GPIO_PIN_RESET)
	{
	  return true;
	}
	return false;
...
{
	if (HAL_GPIO_ReadPin(PS_GPIO_EVERSION_RIGHT, PS_PIN_EVERSION_RIGHT) == GPIO_PIN_RESET)
	{
	  return true;
	}
	return false;
...
{
	if (PeriphSwitch_LegLeft() || PeriphSwitch_LegRight() || PeriphSwitch_ExtensionUp() || PeriphSwitch_ExtensionDown() ||
			PeriphSwitch_DorsiflexionUp() || PeriphSwitch_DorsiflexionDown() || PeriphSwitch_EversionLeft() || PeriphSwitch_EversionRight())
	{
	  return true;
	}
	return false;
