...
class UnityTestSummary:
    ...
    def run(self):
        ...
        for result_file in results:
            ...
            entry_one = Combine(oneOf(list(alphas)) + ":/" + Word(alphanums + "_-./"))

            entry_two = Word(printables + " ", excludeChars=":")
            entry = entry_one | entry_two

            delimiter = Literal(":").suppress()
            tc_result_line = Group(
                entry.setResultsName("tc_file_name")
                + delimiter
                + entry.setResultsName("tc_line_nr")
                + delimiter
                + entry.setResultsName("tc_name")
                + delimiter
                + entry.setResultsName("tc_status")
                + Optional(delimiter + entry.setResultsName("tc_msg"))
            ).setResultsName("tc_line")

            eol = LineEnd().suppress()
            sol = LineStart().suppress()
            blank_line = sol + eol

            tc_summary_line = Group(
                Word(nums).setResultsName("num_of_tests")
                + "Tests"
                + Word(nums).setResultsName("num_of_fail")
                + "Failures"
                + Word(nums).setResultsName("num_of_ignore")
                + "Ignored"
            ).setResultsName("tc_summary")
                    ...
                    if "tc_status" in tmp_tc_line:
                        if str(tmp_tc_line["tc_status"]) == "IGNORE":
                            if "tc_msg" in tmp_tc_line:
                                tmp_tc.add_skipped_info(
                                    message=tmp_tc_line["tc_msg"],
                                    output=r"[File]={0}, [Line]={1}".format(
                                        tmp_tc_line["tc_file_name"],
                                        tmp_tc_line["tc_line_nr"],
                                    ),
                                )
                            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            targets_dir = "./"
        targets = list(
            map(lambda x: x.replace("\\", "/"), glob(targets_dir + "*.test*"))
        )
        if len(targets) == 0:
            raise Exception(
