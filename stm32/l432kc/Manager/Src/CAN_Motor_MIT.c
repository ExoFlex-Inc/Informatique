...
//=-12.5f; 	float P_MAX =12.5f; 	float V_MIN =-50.0f; 	float V_MAX =50.0f; 	float
//T_MIN =-65.0f; 	float T_MAX =65.0f; 	float Kp_MIN =0; 	float Kp_MAX =500; 	float
//Kd_MIN =0; 	float Kd_MAX =5;
//     uint8_t data[8];
//
...
//higher 	data[4] = kp_int & 0xFF; // KP 8 bit lower 	data[5] = kd_int >> 4; // Kd
//8 bit higher 	data[6] = ((kd_int & 0xF) << 4) | (kp_int >> 8); // KP 4 bit
