...
void Unmount_SD(const TCHAR* path)
{
    ...
    //successfully...\n\n\n"); 	else Send_Uart("ERROR!!! in UNMOUNTING SD
    //CARD\n\n\n");
}
...
FRESULT Write_File(char* name, char* data)
{
    ...
    else
    {
        ...
        if (fresult != FR_OK)
        {
            ...
            //fresult, name); 	    	Send_Uart(buf); 	        vPortFree(buf);
            return fresult;
        }
        ...
        else
        {
            ...
            {
                //	    		char *buf = pvPortMalloc(100*sizeof(char));
                //	    		sprintf (buf, "ERROR!!! No. %d while writing to the
                //FILE *%s*\n\n", fresult, name); 	    		Send_Uart(buf);
                //	    		vPortFree(buf);
            }
            ...
            else
            {
                ...
                //successfully\n", name); 	    		Send_Uart(buf); 	    		vPortFree(buf);
            }
        }
        return fresult;
    }
...
char* Read_File(char* name)
{
    ...
    if (fresult != FR_OK)
    {
        // Handle error when the file doesn't exist
        return NULL;
    }
    else
    {
        ...
        char* buffer = pvPortMalloc(f_size(&fil));  // Allocate enough space
        fresult      = f_read(&fil, buffer, f_size(&fil), &br);
        if (fresult != FR_OK)
        {
            // Handle error reading from the file
            vPortFree(buffer);  // Free the allocated buffer
            return NULL;
        }
